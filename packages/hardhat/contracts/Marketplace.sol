...
contract Marketplace {
    ...
    function writeroom(
        ...
        require(bytes(_name).length > 0 && bytes(_image).length > 0 && bytes(_description).length > 0 && bytes(_location).length > 0 && _pricePerNight > 0, "Invalid room parameters");
    ...
    function makeReservation(uint256 _index, uint256 _noOfNights) public payable {
        ...
        uint256 noOfNightsInSeconds = _noOfNights * 1 seconds;
        uint256 bookingFee = rooms[_index].pricePerNight * _noOfNights;
        uint256 totalFee = bookingFee + reservationFee;
        require(IERC20Token(cEURTokenAddress).transferFrom(msg.sender, address(this), totalFee), "Token transfer failed.");
    ...
    function endReservation(uint256 _index) public {
        ...
        require(rooms[_index].owner == msg.sender || rooms[_index].currentReservedTo == msg.sender, "No access");
        uint256 amountToSendToOwner = rooms[_index].currentBookingFee - reservationFee;
        require(IERC20Token(cEURTokenAddress).transfer(rooms[_index].owner, amountToSendToOwner), "Transfer to owner failed.");
        require(IERC20Token(cEURTokenAddress).transfer(rooms[_index].currentReservedTo, reservationFee), "Transfer of reservation fee to user failed.");
