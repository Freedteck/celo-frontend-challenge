...
// Define the AddBookModal component
const AddBookModal = () => {
  ...
  const [debouncedProductAuthor] = useDebounce(productAuthor, 500);
  const [debouncedProductImage] = useDebounce(productImage, 500);
  const [debouncedProductPrice] = useDebounce(productPrice, 500);
  // const [debouncedProductRead] = useDebounce(productRead, 500);
  ...
  // Use the useContractSend hook to use our addBook function on the marketplace contract and add a product to the marketplace
  const { writeAsync: createProduct } = useContractSend("addBook", [
    debouncedProductTitle,
    debouncedProductAuthor,
  ...
  const handleCreateProduct = async () => {
    ...
    // Create the product by calling the addBook function on the marketplace contract
